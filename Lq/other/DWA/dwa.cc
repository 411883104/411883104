    ...
    while (v < dynamic_window_vel[1])
    {
        while (w < dynamic_window_vel[3])
        {
            ...
            double G = heading_eval + dist_eval + vel_eval +
                       _goal(trajectory, goal); // 第3步--轨迹评价
...
int main()
{
    VectorXd state = VectorXd(0.0, 0.0, PI / 8.0, 0.0, 0.0);
    // state << ;             //[x(m), y(m), yaw(rad), v(m/s), omega(rad/s)]

    Vector2d goal(1, 1); // 目标点
    double dt = 0.02;    // 采样时间
    double v_min = -2, v_max = 6.0, w_min = -PI / 2,
           w_max = PI / 2;                       // 线速度角速度边界
    double predict_time = 30.0;                  // 轨迹推算时间长度
    double a_vmax = 1, a_wmax = PI / 4;          // 线加速度和角加速度最大值
    double v_sample = 0.05, w_sample = PI / 180; // 采样分辨率
    double alpha = 0.55, beta = 1, gamma = 1.5;  // 轨迹评价函数系数

    // Vector2d goal(20, 1); // 目标点
    // double dt = 0.1;      // 采样时间
    // double v_min = -0.5, v_max = 1.0, w_min = -40 * PI / 180,
    //        w_max = 40 * PI / 180;                // 线速度角速度边界
