...
void mixColumns(std::vector<unsigned char>& state) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
        tmp[0] = (unsigned char)(state[i] ^ state[4 + i] ^ state[8 + i] ^ state
    for (unsigned int i = 0; i < 4; i++) {
...
std::vector<unsigned char> encrypt(const std::vector<unsigned char>& plainText, const std::vector<unsigned char>& key) {
    ...
    addRoundKey(state, std::vector<unsigned char>(expandedKey.begin(), expandedKey.begin() + 16));

    for (unsigned int round = 0; round < numRounds; round++) {
        subBytes(state);
        shiftRows(state);
        mixColumns(state);
        addRoundKey(state, std::vector<unsigned char>(expandedKey.begin() + (round + 1) * 16, expandedKey.begin() + (round + 2) * 16));
    }

    subBytes(state);
    shiftRows(state);
    addRoundKey(state, std::vector<unsigned char>(expandedKey.begin() + numRounds * 16, expandedKey.end()));
...
std::vector<unsigned char> decrypt(const std::vector<unsigned char>& cipherText, const std::vector<unsigned char>& key) {
    ...
    invShiftRows(state);
    invSubBytes(state);
    addRoundKey(state, std::vector<unsigned char>(expandedKey.begin() + (numRounds - 1) * 16, expandedKey.begin() + numRounds * 16));

    for (int round = numRounds - 1; round > 0; round--) {
        invMixColumns(state);
        invShiftRows(state);
        invSubBytes(state);
        addRoundKey(state, std::vector<unsigned char>(expandedKey.begin() + round * 16, expandedKey.begin() + (round + 1) * 16));
    }
